...
public enum SignMethod implements DictionaryEnum {
    ...
    private static final Map<String, SignMethod> INDEX_MAP = new HashMap<>();
    private static final List<Dictionary> DICTIONARIES = new ArrayList<>();

    static {
        for (SignMethod signMethod : SignMethod.values()) {
            INDEX_MAP.put(signMethod.getValue(), signMethod);
            DICTIONARIES.add(signMethod.getDictionary(signMethod.name(), signMethod.ordinal()));
        }
    }

    SignMethod(String value, String label) {
        this.label = label;
        this.value = value;
    }

    public static SignMethod get(String index) {
