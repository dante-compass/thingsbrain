...
/**
 * <p>Description: {@link HerodotusMqttAccount} 转 {@link MqttAccount} 转换器 </p>
 *
 * @author : gengwei.zheng
 * @date : 2025/4/2 14:34
 */
public class ToMqttAccountConverter extends AbstractToSysEntityConverter<HerodotusMqttAccount, MqttAccount> {

    private final Converter<HerodotusMqttCategory, MqttCategory> toMqttCategory;

    public ToMqttAccountConverter() {
        this.toMqttCategory = new ToMqttCategoryConverter();
    }

    @Override
    public MqttAccount getInstance() {
        return new MqttAccount();
    }

    @Override
    public void prepare(HerodotusMqttAccount source, MqttAccount target) {

        target.setId(source.getAccountId());
