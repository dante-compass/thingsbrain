<p align="center"><img src="./readme/new-logo.png" height="300" width="400" alt="logo"/></p>
<h2 align="center">简洁优雅 · 稳定高效 | 宁静致远 · 精益求精 </h2>
<p align="center">Dante Cloud 生态产品 -- ThingsBrain 物联网平台</p>

---

<p align="center">
    <a href="https://spring.io/projects/spring-boot" target="_blank"><img src="https://img.shields.io/badge/Spring%20Boot-4.1.1-blue.svg?logo=springboot" alt="Spring Boot 4.1.1"></a>
    <a href="https://spring.io/projects/spring-cloud" target="_blank"><img src="https://img.shields.io/badge/Spring%20Cloud-2025.1.3-blue.svg?logo=springboot" alt="Spring Cloud 2025.1.3"></a>
    <a href="https://github.com/alibaba/spring-cloud-alibaba" target="_blank"><img src="https://img.shields.io/badge/Spring%20Cloud%20Alibaba-2025.1.0.0-blue.svg?logo=alibabadotcom" alt="Spring Cloud Alibaba 2025.1.0.0"></a>
    <a href="https://github.com/Tencent/spring-cloud-tencent" target="_blank"><img src="https://img.shields.io/badge/Spring%20Cloud%20Tencent-2.1.2.0--2025.1.1-blue.svg?logo=qq" alt="Spring Cloud Tencent 2.1.2.0-2025.1.1"></a>
    <a href="https://nacos.io/docs/latest/overview/" target="_blank"><img src="https://img.shields.io/badge/Nacos-3.2.4-brightgreen.svg?logo=alibabadotcom" alt="Nacos 3.2.4"></a>
</p>
<p align="center">
    <a href="https://my.oschina.net/pointerv" target="_blank"><img src="https://img.shields.io/badge/Author-%E7%A0%81%E5%8C%A0%E5%90%9B-orange" alt="码匠君"></a>
    <a href="./LICENSE"><img src="https://img.shields.io/badge/License-Apache--2.0-blue.svg?logo=apache" alt="License Apache 2.0"></a>
    <a href="https://bell-sw.com/pages/downloads/#downloads" target="_blank"><img src="https://img.shields.io/badge/JDK-25%2B-green.svg?logo=openjdk" alt="Java 25"></a>
    <a href="https://github.com/dante-compass/thingsbrain" target="_blank"><img src="https://img.shields.io/badge/Version-4.1.1.1-red.svg?logo=spring" alt="ThingsBrain 4.1.1.1"></a>
    <a href="https://github.com/dromara/dante-cloud" target="_blank"><img src="https://img.shields.io/badge/Dante%20Cloud-4.1.1.1-red.svg?logo=spring" alt="Version 4.1.1.1"></a>
    <a href="https://github.com/dante-compass/dante-engine" target="_blank"><img src="https://img.shields.io/badge/Dante%20Engine-4.1.1.1-red.svg?logo=spring" alt="Dante Engine 4.1.1.1"></a>
    <a href="https://github.com/dante-compass/dante-cloud-ui" target="_blank"><img src="https://img.shields.io/badge/Dante%20Cloud%20UI-4.1.1.1-blue.svg?logo=quasar&logoColor=%23050A14" alt="Dante Cloud UI 4.1.1.1"></a>
    <a href="https://github.com/dante-compass/herodotus-cloud-ui-vuetify" target="_blank"><img src="https://img.shields.io/badge/Dante%20Cloud%20UI(New)-4.1.1.1-blue.svg?logo=vuetify&logoColor=%231867C0" alt="Dante Cloud UI(New) 4.1.1.1"></a>
    <a href="https://github.com/dante-compass/thingsbrain"><img src="https://img.shields.io/github/stars/dante-compass/thingsbrain.svg?label=Github%20Stars" alt="Github star"></a>
    <a href="https://github.com/dante-compass/thingsbrain"><img src="https://img.shields.io/github/forks/dante-compass/thingsbrain.svg?label=Github%20Forks" alt="Github fork"></a>
    <a href="https://gitee.com/dante-compass/thingsbrain"><img src="https://gitee.com/dante-compass/thingsbrain/badge/star.svg?theme=dark" alt="Gitee star"></a>
    <a href="https://gitee.com/dante-compass/thingsbrain"><img src="https://gitee.com/dante-compass/thingsbrain/badge/fork.svg?theme=dark" alt="Gitee fork"></a>
    <a href="https://www.herodotus.cn"><img src="https://visitor-badge.laobi.icu/badge?page_id=dante-cloud&title=Total%20Visits" alt="Total Visits"></a>
</p>
<p align="center">
    <a href="https://github.com/dromara/dante-cloud">Github 仓库</a> &nbsp; | &nbsp;
    <a href="https://gitee.com/dromara/dante-cloud">Gitee 仓库</a> &nbsp; | &nbsp;
    <a href="https://www.herodotus.cn">在线文档</a>
</p>

# ThingsBrain 企业级物联网平台

ThingsBrain 是一款基于 Dante Cloud 企业级能力构建的物联网平台。它充分继承了 Dante Cloud 国内首个支持阻塞式和响应式服务并行的微服务云原生基座的全部优势，将微服务架构的最佳实践与物联网场景深度融合，为万物互联与 AI 融合提供高可靠、高安全的一站式物联网应用基座。

ThingsBrain 不仅是一套功能完备的物联网平台，更是 Dante Cloud 全栈能力的最佳价值兑现应用场景——通过真实、复杂的物联网业务场景，深度验证了 Dante Cloud 在企业级应用中的架构弹性、安全可靠性与开发效率。

## 一、为什么构建 ThingsBrain

当前，人工智能技术正处于高速发展期，大模型、边缘智能、多模态感知等技术加速成熟，物联网正在从“连接管理”阶段迈向“智能协同”阶段。 与此同时，物联网平台类产品已较为丰富，既有通用型平台，也有面向特定行业的垂直平台。 在AI高速发展、竞品成熟的当下，新建物联网产品岂不是重复造轮子？

之所以在此时推出 ThingsBrain 物联网平台，主要原因是：

### 1. AI 时代并未“消解”软件系统的复杂性，反而放大了架构短板

2026年，AI编程助手已极大提升了单点编码效率，但在平台建设领域，核心瓶颈早已从“代码编写”转移到了“架构治理”。 以物联网为例，设备接入的协议碎片化（MQTT/CoAP/Modbus/私有TCP）、海量并发下的连接状态管理、以及设备影子与业务系统间的数据一致性，这些根本性的分布式系统难题，并未因AI辅助编码而得到丝毫缓解。

### 2. “成熟”竞品与云原生鸿沟

主流的物联网平台产品虽功能齐全，但在技术选型上滞后于云原生浪潮。 它们在响应式编程、虚拟线程（Project Loom）、细粒度服务治理等新一代 Java 技术栈的适配度不足。 这产生以下主要问题：

- **资源利用率低**：传统阻塞式 I/O 模型在处理万级设备长连接时，线程开销巨大，无法充分利用现代云基础设施的算力。
- **扩展性僵化**：缺乏领域驱动（DDD）的模块化拆解，多协议适配、多存储切换等核心能力往往与业务耦合过紧，难以应对企业不同阶段的弹性需求。

### 3. Dante Cloud 构建产品生态的需要

Dante Cloud 作为云原生微服务基座，已在技术底座层面形成较完整的能力积累，涵盖微服务治理、容器化部署、多租户体系、安全认证、服务监控与弹性伸缩等核心能力。 但 Dante Cloud 的目标并不局限于是一套云原生微服务基座，而是向物联网、人工智能方向发展，成为可以支持更多场景、构建完整应用、拥有典型案例产品的软件生态。

## 二、 ThingsBrain 的差异化优势

ThingsBrain 的构建并非重复造轮子，而是基于 Dante Cloud 的技术优势，将 AI 就绪性视为平台的第一性原理

### 1. 生于云原生，而非“被云原生”

ThingsBrain 并非在传统架构上打补丁，而是从第一天起就生长在 Dante Cloud 这一云原生微服务基座之上。 Dante Cloud已深度适配 Spring Boot 4 和 Spring Cloud 2025.1，为AI时代的技术演进预留了友好的底座支持。 这种“原生”优势，不需要大规模重构就可以拥抱云原生。

### 2. 安全基因，生而强大

Dante Cloud 满足国家三级等保要求，支持接口国密数字信封加解密、防刷、高防 XSS 和 SQL 注入等一系列安全体系。 ThingsBrain 继承了这一完整的安全能力，在设备认证、数据传输、权限控制等环节具备天然的安全优势。

### 3. “一套代码、两种架构”的灵活部署能力

ThingsBrain 继承了 Dante Cloud 独特的架构能力——一套代码既可以单体模式运行，也可以微服务模式运行。 这意味着：中小型项目可以低成本快速启动，随着设备规模和业务复杂度增长，可以无缝演进至分布式微服务架构，无需推翻重来。

### 4. 阻塞式与响应式并行，真正的混合运行时架构

Dante Cloud 是国内首个支持阻塞式服务和响应式服务并行的微服务平台。 这一独特的架构能力意味着： ThingsBrain 在处理传统的设备管理、规则引擎等业务时，可以采用成熟的阻塞式编程模型，保证业务逻辑的清晰与可维护性；而在处理海量设备连接、实时数据流处理、推送服务等高并发场景时，则可以无缝切换至响应式编程模型，以极低的资源消耗支撑数万乃至数十万的并发连接。 两种模式并非互斥的二选一，而是在同一平台、同一业务流中按需并存、协同工作。

### 5.部署形态灵活，三态合一

ThingsBrain具备三种部署与集成形态，赋予企业按需选择的自由：

- **独立部署模式**：ThingsBrain 可作为独立的物联网平台完整部署，拥有独立的网关、认证、存储与运维体系，适用于已有IT基础设施、仅需物联网能力的场景，开箱即用、轻量高效。
- **微服务融合模式**：ThingsBrain 可作为 Dante Cloud 微服务版的一个独立微服务注册至服务发现中心，与 Dante Cloud 既有的认证服务、网关服务、监控服务等无缝协同，共享微服务治理能力（熔断、限流、链路追踪），适用于大型分布式项目。
- **单体模块模式**：ThingsBrain 可直接作为 Dante Cloud 单体版的一个功能模块引入，与主工程共享同一个进程与数据库，无需额外部署与网络开销，适用于中小型项目快速集成物联网能力。

### 6. 可插拔的多协议、多存储架构

ThingsBrain 具备多协议、多存储可插拔架构，能够灵活适配不同行业、不同场景的物联网需求，避免了“一套方案打天下”的僵化。

### 7. 兼容主流平台，支持物联网平台主流功能

为降低用户迁移门槛、保护既有技术投资，ThingsBrain 在功能设计上主动兼容阿里云物联网平台产品，支持物联网平台领域的主流功能与常见交互模式。 具体包括：

- **设备接入兼容**：支持 MQTT、HTTPS 等主流协议，兼容阿里云物联网平台设备接入认证方式与三元组模型（ProductKey、DeviceName、DeviceSecret），使存量设备可平滑迁移或双平台并行运行。
- **物模型兼容**：支持物模型（Thing Model）定义，兼容阿里云物联网平台物模型规范，包括属性、服务、事件三类核心要素，降低设备侧数据模型转换成本。
- **主流功能全覆盖**：完整支持设备生命周期管理、在线状态监控、设备影子、数据解析、规则引擎、数据转发、告警中心、可视化监控、OTA 升级等物联网平台主流功能

### 8. 与AI融合的天然接口

Dante Cloud 4.X已为AI时代的技术演进预留了底座支持，ThingsBrain在此基础上构建，具备从设备接入、数据采集到AI推理的完整数据链路能力，为万物互联与AI融合提供高可靠、高安全的一站式基座。

## 三、建设目标

ThingsBrain 的建设分为三个阶段：

- **近期目标**： 完成物联网平台核心能力建设，包括多协议设备接入、设备生命周期管理、数据采集与存储、规则引擎、告警中心、可视化监控、开放 API，并完成与 Dante Cloud 的深度集成。同时，实现与阿里云物联网平台在设备接入、物模型、核心 API 层面的基础兼容，形成可运行、可使用的首个生态产品。
- **中期目标**：增强边缘计算与 AI 协同能力，支持边缘节点管理、边缘自治、模型下发、云边数据同步，以及基于规则和模型的智能联动。 进一步深化主流平台兼容能力，完善数据迁移工具和规则转换机制，形成面向重点行业的解决方案模板，积累可复用的场景化能力。
- **远期目标**：使 ThingsBrain 成为 Dante Cloud 生态中成熟稳定的物联网产品，支撑多行业、多客户复制推广，并在此过程中沉淀平台能力，反哺 Dante Cloud 基座，形成“基座支撑产品、产品沉淀能力、能力反哺基座”的生态正循环。同时，保持对主流物联网平台功能的持续跟踪与兼容演进，确保生态产品的长期竞争力。

## 四、核心价值

| 价值维度         | 说明                                                                               |
|------------------|------------------------------------------------------------------------------------|
| **降低开发成本** | 内置设备接入、物模型、设备影子等能力，减少重复造轮子，让团队专注于业务创新。       |
| **缩短上线周期** | 开箱即用，快速搭建物联网业务原型并平滑演进到生产环境。                             |
| **提升系统吞吐** | 响应式架构与统一消息体系结合，支撑海量设备并发连接与消息处理，提升资源利用率。     |
| **增强安全可控** | OAuth2 + MQTT 统一认证，方法级动态鉴权，权限可运营、可审计，满足企业安全合规要求。 |
| **保护既有投资** | 单体到微服务平滑迁移，兼容阿里云物联网平台，降低架构演进和生态切换风险。           |
| **降低运维难度** | 统一消息与认证体系，简化系统集成与日常运维，减少故障排查成本。                     |

## 五、应用场景

- 智能硬件与消费物联网
- 工业物联网与智能制造
- 智慧城市与楼宇自动化
- 车联网与物流追踪
- 能源监控与环境监测

## 六、开源协议

### 1. 协议声明

ThingsBrain 项目开源协议为 Apache License Version 2.0。可用于个人学习、毕设，允许商业使用，禁止二次开源。严禁搬运至 CSDN 下载等平台进行售卖。

### 2. 补充条款

使用时务必遵守以下补充条款。

- 不得将本软件应用于危害国家安全、荣誉和利益的行为，不能以任何形式用于非法为目的的行为。
- 在延伸的代码中（修改现有源代码衍生的代码中）需要带有原来代码中的协议、版权声明和其他原作者规定需要包含的说明（请尊重原作者的著作权，不要删除或修改文件中的Copyright和@author信息） ，更不要全局替换源代码中的 Dante Cloud、Dante Engine、ThingsBrain 或 码匠君 等字样，否则你将违反本协议条款承担责任。
- 您若套用本软件的一些代码或功能参考，请保留源文件中的版权和作者，需要在您的软件介绍明显位置 说明出处，举例：本软件基于 Dante Cloud 微服务架构 Dante Engine 或 ThingsBrain，并附带链接：<https://www.herodotus.cn>
- 任何基于本软件而产生的一切法律纠纷和责任，均与作者无关。
- 如果你对本软件有改进，希望可以贡献给我们，双向奔赴互相成就才是王道。
- 本项目已申请软件著作权，请尊重开源。

如果您确实需要删除作者或版权信息，需要争得作者同意及授权。或者在 [【使用公司及组织】](https://gitee.com/dromara/dante-cloud/issues/ICAOHG) 下进行登记，经作者整理登记信息形成表格后，可视为正式授权。

## 七、工程结构

```shell
thingsbrain
├── thingsbrain-application -- ThingsBrain 应用相关模块分组
├    ├── dante-cloud-iot-ability -- ThingsBrain 服务模块(微服务版)
├    └── thingsbrain-monolith-application -- ThingsBrain 物联网平台应用(单体版)
├── thingsbrain-dependencies -- ThingsBrain Bom 定义, 统一管理工程模块
├── thingsbrain-kernel -- ThingsBrain 核心定义相关模块分组
├    ├── thingsbrain-kernel-commons -- 核心定义通用代码模块
├    ├── thingsbrain-kernel-link -- 自定义 Link 协议核心定义代码模块
├    └── thingsbrain-kernel-tsl -- 物模型核心定义代码模块分组
├── thingsbrain-link -- 自定义 Link 协议相关模块分组
├    ├── thingsbrain-link-autoconfigure -- 自定义 Link 协议自动配置模块
├    ├── thingsbrain-link-commons -- 自定义 Link 协议通用代码模块
├    ├── thingsbrain-link-manager -- 自定义 Link 协议管理器模块
├    └── thingsbrain-link-commons -- 自定义 Link 协议上报数据存储模块(时序数据)
├── thingsbrain-mqtt -- Mqtt 业务逻辑相关模块分组
├    ├── thingsbrain-mqtt-autoconfigure -- Mqtt 业务逻辑自动配置模块
├    ├── thingsbrain-mqtt-commons -- Mqtt 业务逻辑通用代码模块
├    ├── thingsbrain-mqtt-inbound -- Mqtt 入站数据业务逻辑实现代码模块
├    └── thingsbrain-mqtt-outbound -- Mqtt 出站数据业务逻辑实现代码模块
├── thingsbrain-nosql -- NoSQL 非结构化数据存储模块分组
├    ├── thingsbrain-nosql-autoconfigure -- 非结构化数据存储自动配置模块
├    └── thingsbrain-nosql-influxdb3 -- InfluxDB3 封装模块
├── thingsbrain-persistence -- 数据持久化相关模块分组
├    ├── thingsbrain-persistence-autoconfigure -- 数据持久化自动配置模块
├    ├── thingsbrain-persistence-commons -- 数据持久化通用代码模块
├    ├── thingsbrain-persistence-jpa -- 以 JPA 作为核心业务数据持久化层实现模块
├    └── thingsbrain-persistence-mongodb -- 以 MongoDB 作为核心业务数据持久化层实现模块
├── thingsbrain-platform -- 平台功能相关模块分组
├    ├── thingsbrain-platform-authentication -- 设备认证功能逻辑模块
├    ├── thingsbrain-platform-autoconfigure -- 平台功能自动配置模块
└──  └── thingsbrain-platform-rest -- 平台功能 REST 接口模块
```

## 八、版本分支

### 1. 版本号说明

本系统版本号，分为四段。

- 第一段、第二段和第三段，与 Spring Boot 版本对应，根据采用的 Spring Boot 版本变更。例如，当前采用 Spring Boot 2.4.6 版本，那么就以
  2.4.6.X 开头
- 第四段，表示在当前 Spring Boot 版本下，系统功能维护及优化情况。

**本系统未采用传统的、从 1.0.0 开始的版本号，主要基于以下两点考虑：一方面，方便了解对应的 Spring Boot 版本；另一方面，与 Dante Cloud 以及 Dante Engine 匹配对应，以减少不必要麻烦。**

### 2. 分支说明

| 分支名称 | 对应 Spring 生态版本                     | 对应 JDK 版本 | 用途             | 现状                                                         |
|:--------:|------------------------------------------|---------------|------------------|--------------------------------------------------------------|
|  master  | Spring Boot 4.1 和 Spring Cloud 2025.1.3 | JDK 25        | 主要发布分支     | 推荐使用代码分支                                             |
| develop  | Spring Boot 4.1 和 Spring Cloud 2025.1.3 | JDK 25        | Development 分支 | 新功能、ISSUE 均以此分支作为开发，发布后会 PR 至 master 分支 |


## 九、关注我

<table align="center">
  <tr>
    <th align="center">
      <p>公众号：码匠君</p>
    </th>
  </tr>
  <tr>
    <td align="center">
      <img src="./readme/公众号.jpg" alt="公众号" height="200px">
    </td>
  </tr>
</table>